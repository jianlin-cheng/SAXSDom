...
//Hierarchies read_pdb_saxs(std::istream& in, std::string name, Model* model, std::string inputString,
Hierarchies read_pdb_saxs(TextInput in, std::string name, Model* model, std::string inputString,
                     PDBSelector* selector, bool select_first_model,
                     bool split_models, bool noradii) {
  IMP_FUNCTION_LOG;
  IMP::PointerMember<PDBSelector> sp(selector);
  ...
  //while (!in.eof()) {
    //getline(in, line);
	//std::cout << "Name: " << name << "in pdb_saxs_jie.cpp: "  << line << std::endl;
    //if (in.eof()) break;
    //if (in.bad() || in.fail()) {
     // IMP_THROW("Error reading from PDB file", IOException);
    //}
...
// added by jie 11/21/2016
void read_pdb_SAXS(TextInput in, int model, std::string inputString, Hierarchy h) {
  boost::unordered_map<int, Particle*> atoms_map;
  atom::Hierarchies atoms = get_by_type(h, ATOM_TYPE);
  boost::unordered_map<core::RigidBody, ParticleIndexes> rigid_bodies;
  for (unsigned int i = 0; i < atoms.size(); ++i) {
  ...
  std::istringstream split(inputString);
  while(std::getline(split, line, ';')) {
	//std::cout << "second in pdb_saxs_jie.cpp: "  << line << std::endl;
    //std::cout << line << '\n';
  //while (!in.get_stream().eof()) {
  //  getline(in.get_stream(), line);
   // if (in.get_stream().eof()) break;
...
// added by jie 11/21/2016
Hierarchy read_pdb_saxs(TextInput in, Model* model, std::string inputString,
                   PDBSelector* selector, bool select_first_model,
                   bool no_radii) {
  IMP::PointerMember<PDBSelector> sp(selector);
  Hierarchies ret = read_pdb_saxs(in, nicename(in.get_name()), model,inputString, selector,
                             select_first_model, false, no_radii);
  if (ret.empty()) {
    IMP_THROW("No molecule read from file " << in.get_name(), ValueException);
  }
