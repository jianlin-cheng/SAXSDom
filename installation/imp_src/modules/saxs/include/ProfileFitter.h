...
class ProfileFitter : public Object {
 ...
 //! compute fit score
  double compute_score_custom(const Profile* model_profile, std::vector<std::string> &fit_profile_array, bool use_offset = false,
                       const std::string fit_file_name = "") const;
  ...
  */
  ...
  // added by jie on 2017/06/16
  FitParameters fit_profile_custom(Profile* partial_profile,  std::vector<std::string> &fit_profile_array, double min_c1 = 0.95,
                            double max_c1 = 1.05, double min_c2 = -2.0,
                           double max_c2 = 4.0, bool use_offset = false,
                            const std::string fit_file_name = "") const;
  ...
  */
  ...
  // added by jie on 2017/06/16
  // writes 3 column fit file, given scale factor c, offset and chi square
  void write_SAXS_fit_file_custom(const std::string& file_name, std::vector<std::string> &fit_profile_array,
                           const Profile* model_profile, const double chi_square,
                           const double c = 1, const double offset = 0) const;
 private:
...
// added by jie on 2017/06/16
template <typename ScoringFunctionT>
FitParameters ProfileFitter<ScoringFunctionT>::fit_profile_custom(
    Profile* partial_profile,std::vector<std::string> &fit_profile_array, double min_c1, double max_c1, double min_c2,
    double max_c2, bool use_offset, const std::string fit_file_name) const {
  ...
  // compute a profile for best c1/c2 combination
  partial_profile->sum_partial_profiles(best_c1, best_c2);
  compute_score_custom(partial_profile,fit_profile_array, use_offset, fit_file_name);
...
// added by jie on 2017/06/16
template <typename ScoringFunctionT>
double ProfileFitter<ScoringFunctionT>::compute_score_custom(
    const Profile* model_profile, std::vector<std::string> &fit_profile_array, bool use_offset,
    const std::string fit_file_name) const {
  IMP_NEW(Profile, resampled_profile,
          (exp_profile_->get_min_q(), exp_profile_->get_max_q(),
           exp_profile_->get_delta_q()));
  ...
  if (fit_file_name.length() > 0) {
    ...
    double c = scoring_function_->compute_scale_factor(exp_profile_,
                                                       resampled_profile,
                                                       offset);
    write_SAXS_fit_file_custom(fit_file_name,fit_profile_array, resampled_profile, score, c, offset);
  }
  return score;
}
...
// added by jie on 2017/06/16
template <typename ScoringFunctionT>
void ProfileFitter<ScoringFunctionT>::write_SAXS_fit_file_custom(
    const std::string& file_name, std::vector<std::string> &fit_profile_array, const Profile* model_profile,
    const double score, const double c, const double offset) const {
  std::ofstream out_file(file_name.c_str());
  if (!out_file) {
    IMP_THROW("Can't open file " << file_name, IOException);
  ...
  for (unsigned int i = 0; i < profile_size; i++) {
	...
	// add to string 
	char bufprofile[300];
    sprintf(bufprofile,"%.5f %.8f %.8f %.5f;", exp_profile_->get_q(i), exp_profile_->get_intensity(i), model_profile->get_intensity(i) * c - offset, exp_profile_->get_error(i));
	//fit_profile_array.append(bufprofile);	
	std::string bufprofile_s = bufprofile;
	fit_profile_array.push_back(bufprofile_s);	
  }
  out_file.close();
}
